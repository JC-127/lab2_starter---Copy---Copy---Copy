...
############# EDIT STUFF BELOW HERE ####################
    ...
    def encrypt(self, msg_str: str) -> bytes:
        ...
        #print("MAC before concatenation: " , mac)

        # concatenate mac and msg here
        msg_bytes += mac
        #print("MAC after concatenation ", mac)
    ...
    def decrypt(self, msg_bytes: bytes) -> str:
        ...
        #plain_byts = plain_byts.decode('utf-8') #allows the message to be converted into a string to see the message
        #print("Plain Message After Decryption and Separation: ", plain_byts)
        #print("HMAC after Decryption and Separation: ", mac)


        # compute and verify mac here
        #plain_byts = plain_byts.encode('utf-8')
        if sha256(plain_byts + self.mac_key).digest() != mac:
            raise Exception("MAC Verification has failed")
            
        elif sha256(plain_byts + self.mac_key).digest() == mac:      
            print("Calculated MAC: ", sha256(plain_byts + self.mac_key).digest() ) #Self Calculated HMAC
            print("MAC from sender: ", mac)
            print("MAC verificaion successful")
    
        else:
            compare_digest( (sha256(plain_byts + self.mac_key).digest()), mac )
            print("Calculated MAC: ", sha256(plain_byts + self.mac_key).digest() )
            print("Mac from sender: ", mac)
